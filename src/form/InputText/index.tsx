...
interface InputTextProps
  extends Omit<InputHTMLAttributes<HTMLInputElement>, OmittedProps> {
  ...
  (
    ...
    const validatePattern = (input: HTMLInputElement) => {
      ...
      if (validity.patternMismatch) {
        updateErrorState(
          input,
          validationMessage?.pattern ?? VALIDATION_MESSAGES.pattern,
        );
        return false;
      }
      return true;
    ...
    const validateRequired = (input: HTMLInputElement) => {
      ...
      if (validity.valueMissing) {
        updateErrorState(
          input,
          validationMessage?.required ?? VALIDATION_MESSAGES.required,
        );
        return false;
      }
      return true;
    ...
    const validateMax = (input: HTMLInputElement) => {
      ...
      if (validity.tooLong) {
        updateErrorState(
          input,
          validationMessage?.max ?? VALIDATION_MESSAGES.max,
        );
        return false;
      }
      return true;
    ...
    const validateMin = (input: HTMLInputElement) => {
      ...
      if (validity.tooShort) {
        updateErrorState(
          input,
          validationMessage?.min ?? VALIDATION_MESSAGES.min,
        );
        return false;
      }
      return true;
      ...
      <>
        <InputCell>
          <input
            ...
            type="text"
            className="dsb-input dsb-input-text--basic"
